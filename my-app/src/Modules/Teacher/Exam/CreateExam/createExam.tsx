...
import convertToJSON from "../Editor/Utils/formatExam";
import NewExam from "./libs/interface";
...
const CreateExam = () => {
  ...
  const [selectedGrade, setSelectedGrade] = useState<Tab>({} as Tab);
  const [selectedSubject, setSelectedSubject] = useState<Tab>({} as Tab);
  const [selectedPurpose, setSelectedPurpose] = useState<Tab>({} as Tab);
  ...
  const handleSubmit = async () => {
    ...
    if (response.ok) {
      ...
      const hashId = responseData.hashId;

      console.log("response: ", responseData);
      navigate(`/teacher/exam/config-exam-online/${hashId}`);
    }
  };
      ...
      <form action="">
        <div className="space-y-4 rounded-md bg-white p-5 shadow-md">
          ...
          <PurposeDropdown
            ...
            setSelectValue={setSelectedPurpose}
          />

          <div className="grid grid-cols-12 gap-10">
            <div className="col-span-5">
              <span className="mb-2 flex text-sm font-medium">
                Cấu hình giá
              </span>

              <div className="mb-2 text-xs text-gray-500">
                Chức năng này giúp giáo viên, người tạo đề có thể đặt giá thu
                phí người tham gia thi, Azota đóng vai trò trung gian cung cấp
                nền tảng và thu hộ, hàng tháng Azota sẽ đối soát và chuyển khoản
                về tài khoản ngân hàng của giáo viên/người tạo đề.{" "}
                <span className="text-red-600">
                  Lưu ý: Chắc chắn rằng bạn đã đủ 18 tuổi để sử dụng chức năng
                  này
                </span>
          ...
          <div>
            ...
            <textarea
              ...
              onChange={(e) =>
                handleChangeTextInput("examDescribe", e.target.value)
              }
            ></textarea>
          </div>
        </div>
