...
const QuestionBox: React.FC<QuestionBoxProps> = (props) => {
  ...
  const { topic, options, rightAnswer } =
    examJSON[partKey]["questions"][questionKey];
  ...
  const handleChangeRightAnswer = (
    ...
    newExamJSON[partKey]["questions"][questionKey]["options"][optionKey][
      "isAnswer"
    ] = !isAnswer;
    ...
    <div className="w-full rounded-md border border-gray-400 bg-white py-6">
      ...
      <div className="mt-5 border-t border-slate-300 px-7">
        ...
        <div className="text-[13px]/5 text-slate-600">
          ...
          <div>
            Cách giải: Trong cuộc khai thác thuộc địa lần thứ hai ở Đông Dương
            (1919-1929), thực hiện Pháp tập trung đầu tư vào đồn điền cao su.
          </div>
        </div>
      </div>
    </div>
