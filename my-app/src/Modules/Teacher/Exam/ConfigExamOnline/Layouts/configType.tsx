...
const ConfigType: React.FC<ConfigTypeProp> = (props) => {
  ...
  const [examType, setExamType] = useState<string>(examConfig.examType);

  const toggleSwitch = (newExamType: string) => {
        ...
        <div className="w-full">
          {examType === "TEST" ? (
            <div className="mt-2 text-sm/6 text-slate-500">
              Cấu hình này thường dùng cho các kỳ thi nghiêm túc, cần bảo mật
              đáp án và đề thi cho tới khi kết thúc kỳ thi
            </div>
          ) : (
            <div className="mt-2 text-sm/6 text-slate-500">
              Cấu hình này thường dùng cho học sinh luyện tập. Câu hỏi được hiển
              thị từng câu một giống các game 'Ai là triệu phú', khi trả lời tới
              câu hỏi nào thì học sinh sẽ nhận được đáp án đúng và lời giải của
              câu hỏi đó luôn.{" "}
              <span className="text-[#f97316]">
                Lưu ý: Hiện chưa hỗ trợ luyện tập với câu tự luận.
              </span>
            </div>
          )}
        </div>
